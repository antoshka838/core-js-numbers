...
function getDistanceBetweenPoints(x1, y1, x2, y2) {
  const resX = (Math.abs(x1) + Math.abs(x2)) ** 2;
  const resY = (Math.abs(y1) + Math.abs(y2)) ** 2;
  const result = Math.sqrt(resX + resY);
  return result;
}
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const den1 = Math.sqrt(x1 ** 2 + y1 ** 2);
  const den2 = Math.sqrt(x2 ** 2 + y2 ** 2);
  const result = Math.acos(prod / (den1 * den2));
  return result;
}
